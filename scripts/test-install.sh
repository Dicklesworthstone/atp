...
assert_no_network
pass "checksummed archive validates and installs successfully"
...
run_installer explicit-no-verify --offline "$FIXTURES/valid.tar.gz" --no-verify --dest "$dest" --force
expect_rc 0 "explicit checksum bypass"
[ -x "$dest/atp" ] || fail "--no-verify did not install"
assert_no_network
pass "explicit --no-verify remains the only checksum bypass"
...
RUN_PATH="$AARCH64_FALLBACK_BIN:$ORIGINAL_PATH"
